...
namespace Antares.Ext
{
    ...
    /// In C++, that header provides a compatibility alias for an optional/nullable type
    /// (either `std::optional` or `boost::optional`).
    /// <para>
    /// In C#, this concept is a fundamental language feature and does not require a special class.
    ...
    ///     <description>
    ///       Use a nullable value type, `System.Nullable<T>`, with the C# shorthand `T?`.
    ///       For example, `optional<double>` becomes `double?`.
    ///     </description>
    ///   </item>
    ///   <item>
    ///     <term>C++ `optional<T>` where T is a reference type (e.g., `MyClass*`)</term>
    ///     <description>
    ///       Use a standard reference type, which is inherently nullable in C#.
    ///       For example, `optional<MyClass>` becomes a reference of type `MyClass` which can be assigned `null`.
    ///       With C# 8+ and nullable reference types enabled, this would be `MyClass?`.
    ///     </description>
    ///   </item>
    ///   <item>
    ...
    public static class Optional
    {
        ...
        // equivalents of C++ optional types and prevent its usage.
    }
}